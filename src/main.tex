...
[公司|职位｜最好一次绩效]
{贝壳找房（北京）科技有限公司}
[\textnormal{高级工程师（P6）|\textbf{ 2024年绩效5（卓越）、2025年绩效4（优秀）}}]
[2022.03—至今]
\begin{itemize}
  ...
  \item \textbf{领导力：}企业文化方向研发团队（4人）中担任技术负责人，负责需求拆解、方案评审与交付质量把控，组织CodeReview，保障多系统稳定迭代。
\end{itemize}
...
\begin{itemize}
  ...
  \item 使用Druid对复杂业务规则的CASE-WHEN语句进行AST构建，通过DFS拆解（AND/OR/NOT、德摩根定律），得到原子的CASE-WHEN语句集合。
  \item 基于原子语句集合，实现Hive→Java解析器，统一离线\&实时标签规则的计算逻辑。
  \item 基于原子语句集合，配合LLM，实现Hive→自然语言解析器，统一技术代码与规则语言。
  \item 设计快照类型数据转时间轴算法（基于开窗函数与偏移量计算）实现历史标签高效追溯与查询，使得MySQL可支撑上亿级（逻辑数据量）标签数据的高效读取。
